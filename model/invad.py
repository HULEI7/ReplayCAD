...
class InvAD(nn.Module):
    def __init__(self, model_encoder, model_fuser, model_decoder, model_disor=None):
        ...
        # self.linear1 = nn.Sequential(nn.Linear(256, 64),
        #                              nn.ReLU(),
        #                                 nn.Linear(64, 16),
        #                              nn.ReLU(),)
        # self.linear2 = nn.Sequential(nn.Linear(1024, 128),
        #                              nn.ReLU(),
        #                             nn.Linear(128, 32),
        #                              nn.ReLU(),)
        # self.linear3 = nn.Sequential(nn.Linear(4096, 512),
        #                              nn.ReLU(),
        #                              nn.Linear(512, 64),
        #                              nn.ReLU(),)

        # self.linear4 = nn.Sequential(nn.Linear(7168, 1024),
        #                              nn.ReLU(),)
    ...
    def forward(self, imgs):
        ...
        #     features_pooled = feats_fusion[i].view(feats_fusion[i].shape[0], feats_fusion[i].shape[1],-1)
        #     layer_name = f'linear{i+1}'
        #     linear = getattr(self, layer_name, None)
        #     features_pooled = linear(features_pooled)
        #     features_pooled = features_pooled.view(features_pooled.shape[0], -1)
        #     feats_linear.append(features_pooled)
        
        # feats_linear = torch.cat(feats_linear, dim=1)
        # feats_linear = self.linear4(feats_linear)
        
        # feats_linear = F.normalize(feats_linear, dim=1)
        # feats_linear = feats_linear.unsqueeze(1)
        
        # return feats, feats_pred,feats_linear
