...
class L2Loss(nn.Module):
    def __init__(self, lam=1):
        ...
        # self.loss = nn.SmoothL1Loss(reduction='mean', beta=1.0)  # 使用Smooth L1损失
    ...
    def forward(self, input1, input2):
        ...
        for in1, in2 in zip(input1, input2):
            ...
            # mse_loss = (in1 - in2) ** 2
            # mask = mse_loss < 500  # 创建掩码，标记损失小于阈值的元素
            # truncated_loss = mse_loss * mask  # 将超过阈值的损失设置为0
            # loss += truncated_loss.mean() * self.lam

            # mse_loss = (in1 - in2) ** 2
            # # 计算指定百分位数的阈值
            
            # num_elements = mse_loss.numel()
            # k = int((1 - 0.01) * num_elements)
            # topk_values, _ = torch.topk(mse_loss.view(-1), k, largest=False)
            
            # mask = mse_loss < topk_values[-1] # 创建掩码，标记损失小于阈值的元素
            # truncated_loss = mse_loss * mask  # 将超过阈值的损失设置为0
            # loss += truncated_loss.mean() * self.lam
