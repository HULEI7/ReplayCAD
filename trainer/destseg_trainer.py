...
class DeSTSegTrainer(BaseTrainer):
    def __init__(self, cfg):
        ...
        self.optim.de_st = get_optim(cfg.optim.de_st.kwargs, self.net.module.destseg.student_net,
                                       lr=cfg.lr*40)
        self.optim.seg_optimizer = torch.optim.SGD(
            [{"params": self.net.module.destseg.segmentation_net.res.parameters(), "lr": cfg.lr*10},
                {"params": self.net.module.destseg.segmentation_net.head.parameters(), "lr": cfg.lr},],
            lr=0.001,
            momentum=0.9,
            weight_decay=1e-4,
            nesterov=False,
    ...
    def optimize_parameters(self):
        ...
        if self.epoch < (self.epoch_full / 5):
            self.net.module.destseg.student_net.train()
            self.net.module.destseg.segmentation_net.eval()
        else:
            self.net.module.destseg.student_net.eval()
            self.net.module.destseg.segmentation_net.train()
        ...
        if self.epoch < (self.epoch_full / 5):
            ...
            params = self.net.module.destseg.student_net.parameters()
            loss = loss_cos
        else:
            optims = self.optim.seg_optimizer
            params = list(self.net.module.destseg.segmentation_net.res.parameters()) + list(self.net.module.destseg.segmentation_net.head.parameters())
